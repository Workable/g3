        ...
        messages = [
            {
                ...
                f"Include in your reponse the following: {self.include}. "
                if self.include
                else "" f"Incorporate the following Jira ticket: {self.jira}. "
                if self.jira
                else "" f"Use the following tone when creating the commit message: {self.tone}. "
                if self.tone
                else "" "Your response should be in markdown format.",
            },
            {
                "role": "user",
            ...
            {"role": "user", "content": f"Please provide a commit message for the provided code. Code: ```{code}```"},
        ]
